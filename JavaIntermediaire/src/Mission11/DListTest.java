...
public class DListTest extends junit.framework.TestCase {
	...
	public void testContains() {
		...
		assertEquals("Verification de la presence d'un element present une fois dans la liste", true,
				l.contains(new Double(2.3)));
		assertEquals("Verification de la presence d'un element absent de la liste", false, l.contains(new Double(2.2)));
		assertEquals("Verification de la presence d'un element present deux fois dans la liste", true,
				l.contains(new Double(2.4)));

		// Verification que l'element supprime n'est plus contenu dans la liste

		l.remove(new Double(2.3));
		assertEquals("Verification de la presence d'un element qui n'est plus present dans la liste", false,
				l.contains(new Double(2.3)));
	...
	public void testRemove() {
		...
		assertEquals("Retrait d'un element ne se trouvant plus trouvant dans la liste", false,
				l.remove(new Double(2.3)));
		assertEquals("Retrait d'un element ne se trouvant trouvant pas dans la liste", false,
				l.remove(new Double(2.2)));
